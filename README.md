...
> [!WARNING]
> 即使完成本仓库的工程闸门，系统也不等于获得医疗器械注册或医院临床上线许可。当前只能作为“受控临床试点 / 待医院审批”使用；真实患者分诊前必须完成本院临床安全验证、医生复核闭环、伦理与隐私评审、信息中心验收、统一身份/MPI/医疗关系接入和正式授权。
...
- **上线治理闸门**：模型权重摘要/签名、脱敏临床评测、知识来源登记（国内官方和本院制度优先）、提示词/恶意附件红队、漂移/GPU 基线、变更审批/回滚和安全事件 CAPA 统一记录在 `/api/governance/**`；未通过审核的模型或知识不会进入临床运行。
